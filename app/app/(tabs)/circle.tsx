...
export default function CircleScreen() {
  ...
  const displayItems = useMemo(() => {
    if (onlyLendable) {
      return memberItems.filter((i) => i.is_lendable && !i.is_private);
    }
    return memberItems;
          ...
          >
            <View style={styles.memberHeader}>
              ...
              <View style={{ flex: 1, marginLeft: spacing.md }}>
                ...
                <Text style={[styles.memberCount, { color: colors.textSecondary }]}>
                  {selectedMember.item_count} items in collection
                </Text>
              </View>
            </View>
            ...
            <TouchableOpacity
              ...
              style={[
                styles.filterToggle,
                {
                  backgroundColor: onlyLendable ? colors.accent : colors.surface,
                  borderColor: colors.border,
                },
              ]}
            >
              <MaterialCommunityIcons
                name={onlyLendable ? 'check-circle' : 'check-circle-outline'}
                size={16}
                color={onlyLendable ? colors.charcoal : colors.textSecondary}
              />
              <Text
                style={[
                  styles.filterText,
            ...
            {loadingItems ? (
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <CircleItemCard item={item} onPress={handleItemPress} />
                )}
              />
            )}
              ...
              >
                <Card style={styles.memberCard}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={[styles.memberListName, { color: colors.textPrimary }]}>
                        {member.full_name}
                      </Text>
                      <Text style={[styles.memberListCount, { color: colors.textSecondary }]}>
                        {member.item_count} items
                      </Text>
                    </View>
                    <MaterialCommunityIcons
                      name="chevron-right"
...
function CircleItemCard({ item, onPress }: { item: Item; onPress: (item: Item) => void }) {
  const colors = useThemeColors();
    ...
    >
      <View style={{ position: 'relative' }}>
        ...
        <View style={styles.gridBadgeRow}>
          {item.is_private && (
            <View style={[styles.gridLock, { backgroundColor: colors.surface }]}>
              <MaterialCommunityIcons name="lock" size={11} color={colors.textSecondary} />
            </View>
          )}
          {!item.is_lendable && (
            <View style={[styles.gridNoLend, { backgroundColor: colors.surfaceElevated }]}>
              <Text style={[styles.gridNoLendText, { color: colors.textSecondary }]}>
                Not for lending
              </Text>
            </View>
          )}
      ...
      <View style={styles.gridInfo}>
        <Text style={[styles.gridBrand, { color: colors.accent }]} numberOfLines={1}>
          {item.brand.toUpperCase()}
        </Text>
        <Text style={[styles.gridModel, { color: colors.textPrimary }]} numberOfLines={1}>
          {item.model_name || '—'}
        </Text>
        <Text style={[styles.gridPrice, { color: colors.textSecondary }]}>
