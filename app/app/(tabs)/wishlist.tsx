    ...
    <>
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <FlatList data={tab === "mine" ? myItems : friendItems} keyExtractor={(item) => item.id} contentContainerStyle={styles.list} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.accent} />}
          ListEmptyComponent={loading ? (<View style={{ gap: spacing.md, padding: spacing.lg + 6 }}>{[1,2].map((i) => (<Card key={i}><Skeleton width={150} height={16} style={{ marginBottom: 8 }} /><Skeleton width={100} height={12} style={{ marginBottom: 12 }} /><Skeleton width="100%" height={6} borderRadius={3} /></Card>))}</View>) : (<EmptyState icon={tab === "mine" ? "heart-outline" : "account-group-outline"} title={tab === "mine" ? "No Wishlist Yet" : "No Dreams to Share"} subtitle={tab === "mine" ? "Tap the + button to add your first dream piece" : "Your circle's wishlist items will appear here"} />)}
