...
import { formatCurrency, formatEnum, formatRelativeTime } from '@/lib/format';
    ...
    <>
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={[styles.tabBar, { backgroundColor: colors.surfaceElevated }]}>
          ...
          >
            <Text
              style={[
                styles.tabText,
                { color: tab === 'mine' ? colors.textPrimary : colors.textSecondary },
              ]}
            >
              My Wishlist
            </Text>
          ...
          <TouchableOpacity
            ...
            style={[
              styles.tab,
              { backgroundColor: tab === 'friends' ? colors.surface : 'transparent' },
            ]}
          >
            <Text
              style={[
                styles.tabText,
                { color: tab === 'friends' ? colors.textPrimary : colors.textSecondary },
              ]}
            >
              Friends' Dreams
            </Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) =>
            tab === 'mine' ? (
              <MyWishlistCard item={item} />
            ) : (
              <FriendWishlistCard item={item} />
            )
          }
        />
...
function MyWishlistCard({ item }: { item: WishlistItem }) {
  const colors = useThemeColors();
  const progress = item.target_price
    ? Math.min((item.current_savings / item.target_price) * 100, 100)
    : 0;
  ...
  return (
    <Card style={styles.wishlistCard}>
      <View style={styles.wishlistHeader}>
        <View style={{ flex: 1 }}>
          <Text style={[styles.wishlistBrand, { color: colors.accent }]}>
            {item.brand.toUpperCase()}
          </Text>
          <Text style={[styles.wishlistModel, { color: colors.textPrimary }]} numberOfLines={1}>
            {item.model_name || '—'}
          </Text>
        </View>
        {item.fulfilled && (
      ...
      {item.target_price && (
        <View style={styles.progressSection}>
          <View style={styles.progressInfo}>
            <Text style={[styles.progressSaved, { color: colors.accent }]}>
              {formatCurrency(item.current_savings, item.currency)}
            </Text>
            <Text style={[styles.progressTarget, { color: colors.textSecondary }]}>
              of {formatCurrency(item.target_price, item.currency)}
            </Text>
          </View>
      ...
      {/* Notes */}
      {item.notes && (
        <Text style={[styles.wishlistNotes, { color: colors.textSecondary }]} numberOfLines={2}>
          {item.notes}
        </Text>
      )}
    ...
    <Card style={styles.wishlistCard}>
      ...
      <View style={styles.friendHeader}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.friendTime, { color: colors.textSecondary }]}>
      ...
      <View style={styles.friendItemRow}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[styles.friendBrand, { color: colors.accent }]}>
            {item.brand.toUpperCase()}
          </Text>
          <Text style={[styles.friendModel, { color: colors.textPrimary }]} numberOfLines={1}>
            {item.model_name || '—'}
          </Text>
          {item.target_price && (
            <Text style={[styles.friendPrice, { color: colors.textSecondary }]}>
              Target: {formatCurrency(item.target_price, item.currency)}
            </Text>
          )}
        </View>
      </View>
      ...
      {item.notes && (
        <Text style={[styles.friendNotes, { color: colors.textSecondary }]}>
          {item.notes}
        </Text>
      )}
      ...
      <View style={styles.friendActions}>
        ...
        >
          <MaterialCommunityIcons
            ...
            size={16}
            color={reacted ? colors.accent : colors.textSecondary}
          />
          <Text style={[styles.friendActionText, { color: reacted ? colors.accent : colors.textSecondary }]}>
            React
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
      ...
      <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={onClose}>
        ...
        >
          ...
          <View style={styles.modalField}>
            ...
            <TextInput
              style={[styles.modalInput, styles.modalTextArea, { backgroundColor: colors.surfaceElevated, color: colors.textPrimary }]}
              value={notes}
          ...
          >
            ...
            <Text style={[styles.privacyText, { color: colors.textSecondary }]}>
              {isPrivate ? 'Private' : 'Visible to circle'}
            </Text>
          </TouchableOpacity>

          <View style={{ marginTop: spacing.lg }}>
            <PrimaryButton
              label={submitting ? 'Adding...' : 'Add to Wishlist'}
              onPress={handleSubmit}
              disabled={!brand.trim() || submitting}
            />
          </View>
        </View>
      </TouchableOpacity>
